...
def train():
    ...
    while True:
        for input, loc, label in dataloader:
            ...
            if iteration % 10 == 0:
                summary.add_scalar(
                    'loss/loc_loss', loc_loss.data[0], iteration)
                summary.add_scalar(
                    'loss/conf_loss', conf_loss.data[0], iteration)
                summary.add_scalars(
                    'loss/loss',
            ...
            if iteration % 5000 == 0 and iteration != opt.start_iter:
                print('Save state, iter: {}, Loss:{}'.format(
                    iteration, loss.data[0]))
                if not os.path.isdir('weights'):
                        os.mkdir('weights')
                torch.save(model.state_dict(), 'weights/{}_0712_{}.pth'.format(
                    cfg.get('name', 'SSD'), iteration))
...
def test():
    ...
    for i in range(len(dataset)):
        ...
        boxes, labels, scores = encoder.decode(
            xloc, xconf, nms_thresh=0.5, conf_thresh=0.01)

        pred_bboxes.append(boxes)
        pred_labels.append(labels)
        pred_scores.append(scores)

    print(eval_voc_detection(pred_bboxes, pred_labels, pred_scores,
                             gt_bboxes, gt_labels,
