...
def transform_state(t, **kwargs):
    if callable(t):
        ...
        if 'border' in kwargs and 'border' in t_vars:
            t.__dict__['border'] = BorderTypes.get(
                kwargs['border'], cv2.BORDER_REPLICATE)
...
def HalfBlood(img, anchor, f1, f2):
    assert (isinstance(f1, types.LambdaType) and
            isinstance(f2, types.LambdaType))
...
class RandomErasing(object):
    def __init__(self, area_range=(0.02, 0.2), ratio_range=[0.3, 1 / 0.3],
                 fillval=None, prob=0.5, num=1, anchor=None,
        ...
        for _ in range(10):
            ...
            if tw <= w and th <= h:
                ...
                img = HalfBlood(
                    img, self.anchor,
                    # erase
                    lambda im: self._fill(im, (x1, y1, x1+tw, y1+th), fillval),
                    # cut
                    lambda im: self._fill(im, (x1, y1, x1+tw, y1+th), 0))
...
def _expand(img, size, lt, val):
    ...
    # expand = cv2.copyMakeBorder(
    #     img, y1, nh-h-y1, x1, nw-w-x1,
...
class Pad(object):
    ...
    def __call__(self, img, cds=None):
        ...
        img = HalfBlood(
            ...
            # pad
            lambda im: _expand(
                im, (w + pw*2, h + ph*2), (pw, ph), self.fillval),
            # purer
            lambda im: _expand(im, (w + pw*2, h + ph*2), (pw, ph), 0))
...
class Expand(object):
    ...
    def __call__(self, img, cds=None):
        ...
        img = HalfBlood(
            img, self.anchor,
            # expand
            lambda im: _expand(im, (nw, nh), (x1, y1), self.fillval),
            # purer
            lambda im: _expand(im, (nw, nh), (x1, y1), 0))
