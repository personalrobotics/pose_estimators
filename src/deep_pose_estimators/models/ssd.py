...
class SSD300(nn.Module):
    config = {
        ...
        # prior_variance imply relative weights of position offset,
    ...
    def __init__(self, n_classes):
        ...
        # decrease prediction layers' influence on backbone
        ...
        for i, ar in enumerate(self.config['aspect_ratios']):
            n = len(ar) + 1
            self.Loc.append(nn.Conv2d(
                n_channels[i], n * 4, 3, padding=1))
            self.Conf.append(nn.Conv2d(
                n_channels[i], n * (self.n_classes + 1), 3, padding=1))
    ...
    def _prediction(self, xs):
        ...
        for i, x in enumerate(xs):
            ...
            conf = self.Conf[i](x)
            conf = conf.permute(0, 2, 3, 1).contiguous().view(
                conf.size(0), -1, self.n_classes + 1)
            confs.append(conf)
...
class SSD512(SSD300):
    ...
    def __init__(self, n_classes):
        ...
        self.pred_layers = [
            'conv4_3', 'conv7', 'extra1_2', 'extra2_2', 'extra3_2',
            'extra4_2', 'extra5_2']
